...
class DROP(Evaluator):
    ...
    def extract_answer(self, text: str) -> str:
        ...
        return text
    def extract_answer2(self, text: str) -> str:
    ...
    def local_evaluate(self, model, tokenizer, lora_name: str, lora_path: str, split: str, **kwargs):
        model_lora = PeftModel.from_pretrained(model, lora_path)
        merged_model = model_lora.merge_and_unload(noise=True)
        merged_model.to('cuda').eval()
        ...
        for i in range(0, len(data), batch_size):
            ...
            for j in range(i, min(i + batch_size, len(data))):
                ...
                predicted_answer = self.extract_answer2(text=response_text)
                em_score, f1_score = drop_metric(predicted_answer, reference_answer)
